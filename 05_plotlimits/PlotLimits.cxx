...
void PlotLimits(){
  ...
  TString filename = "../04_thetaoutput/Limits.txt";
  ...
  for(unsigned int i=0; i<n_points; i++){
    double value;
    myfile >> value;
    expected_low_68.emplace_back(value);
  }
  for(unsigned int i=0; i<n_points; i++){
    double value;
    myfile >> value;
    expected_high_68.emplace_back(value);
  }
  for(unsigned int i=0; i<n_points; i++){
    double value;
    myfile >> value;
    expected_low_95.emplace_back(value);
  }
  for(unsigned int i=0; i<n_points; i++){
    double value;
    myfile >> value;
    expected_high_95.emplace_back(value);
  }
  for(unsigned int i=0; i<n_points; i++){
    double value;
    myfile >> value;
    observed.emplace_back(value);
  }
  myfile.clear( );
  ...
  g_expected_95->SetMaximum(100);
  g_expected_95->SetMinimum(5E-4);
  ...
  //10) CMS tags
  TString infotext = "XXX fb^{-1} (YY TeV)";
  ...
  h->SetXTitle("M_{Sig} [GeV]");
  h->SetYTitle("#sigma_{Signal} #times #bf{#it{#Beta}}^{2} [pb]");
